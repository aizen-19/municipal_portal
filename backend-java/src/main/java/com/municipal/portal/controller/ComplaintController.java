...
public class ComplaintController {
    ...
    @PostMapping
    public ResponseEntity<?> registerComplaint(@RequestBody Map<String, String> body, HttpServletRequest request) {
    ...
    public ResponseEntity<List<Complaint>> getComplaints(HttpServletRequest request) {
        String userEmail = (String) request.getAttribute("userEmail");
        List<Complaint> userComplaints = complaintRepository.findByUserEmailIgnoreCaseOrderByDateDesc(userEmail);
        return ResponseEntity.ok(userComplaints);
    }

    @PostMapping("/{id}/simulate-action")
    public ResponseEntity<?> simulateAction(@PathVariable String id, @RequestBody Map<String, String> body, HttpServletRequest request) {
        String status = body.get("status");
        String userEmail = (String) request.getAttribute("userEmail");

        if (status == null || (!status.equals("In Progress") && !status.equals("Resolved"))) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body(Map.of("message", "Invalid status."));
        }

        Optional<Complaint> complaintOpt = complaintRepository.findById(id);
        if (complaintOpt.isEmpty() || !complaintOpt.get().getUserEmail().equalsIgnoreCase(userEmail)) {
